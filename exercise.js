...
class Tool {
	constructor(svg_element_id) {
		...
		d3.selectAll('.dots').attr('fill', function(d) {return color_dots(d)}).attr('stroke-opacity', function(d) {return stroke_opacity(d)})
		...
		this.svg.append('circle')
			...
			.on("click", function() {
				if (flag == 0) {
					d3.select('.horizontal_rect').attr('fill', '#3CB043').attr('opacity', 1)
					d3.select('.vertical_rect_2').attr('fill', '#3CB043').attr('opacity', 1)
		...
		this.svg.append('text')
				...
				.text((TP+FP).toString() + ' tests positifs')
				.attr('opacity', 0)
		...
		function color_dots(d) {
			...
			if (contain(d3.select('.vertical_rect_2'), d)){
				TP += 1
				return color_rect_vert_2
			} 
			if (contain(d3.select('.horizontal_rect'), d)){
				FP += 1
				return color_rect_hor
			} else {
				TN += 1
